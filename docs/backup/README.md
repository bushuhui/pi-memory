...
The built-in `memory-lancedb` plugin in OpenClaw provides basic vector search. **pi-memory** takes it much further:

| Feature | Built-in `memory-lancedb` | **pi-memory** |
|---------|--------------------------|----------------------|
| Vector search | ✅ | ✅ |
...
**Common mistake:** cloning the plugin somewhere else, while keeping a **relative path** like `plugins.load.paths: ["plugins/pi-memory"]`. Relative paths can be resolved against different working directories depending on how the Gateway is started.
...
```bash
# Look for: plugins.slots.memory = "pi-memory"
openclaw config get plugins.slots.memory
```
...
# - Otherwise, check: `openclaw plugins info pi-memory` and locate the directory.
...
Tip: start the message with `run ...` so `pi-memory`'s adaptive retrieval will skip auto-recall injection (saves tokens).
